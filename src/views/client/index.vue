	...
	<div ref="rootDiv" class="rootContainer" @click="closeDialog">
		...
		<div class="cnt-top" ref="top">
			<div class="top-left">
				...
				<el-button
					:type="isHighSearch?'success':'primary'"
					size="small"
					style="margin-left:20px"
					@click.stop="openHighSearch"
				>
					{{isHighSearch?'清空高级检索':'高级检索'}}
				</el-button>
		...
		<high-search v-if="paramsDialog.show" :views="paramsDialog" :params="highParams" @selected="searchByParams"></high-search>
	</div>
</template>
<script>
...
export default {
  ...
  computed: {
	  isHighSearch(){
		  for(let key in this.highParams){
			  if(this.highParams)
			  	return true;
		  }
		  return false;
  ...
  methods: {
    ...
    getList() {
      ...
      para = Object.assign({}, para, this.highParams);
    ...
    showBag(row) {
      ...
      this.$http(url, {
        ...
        this.$set(
          this.bag.data,
          "address",
          row.province + row.city + row.town + row.dz
        );
      ...
      return h(
        ...
        [
          ...
          h("i", {
            ...
            attrs: {
              title:
                "获取客户硬件信息后，当客户在门店盒子的辐射将获取到门店活动信息"
            }
          })
        ]
      );
